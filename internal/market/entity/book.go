...
func (oq *OrderQueue) GetNextOrder() *Order {
	...
	order := (*oq)[0]
	*oq = (*oq)[1:] // ? pulamos a primeira ordem da fila e pegamos todo o restante
	return order
}

func (b *Book) Trade() {
	buyOrders := make(map[string]*OrderQueue) // string se refere ao simbolo do ativo, ou seja, a chave da lista de ordens na fila eh o ativo
